...
from .figshare_eml import load_xwalk, create_eml
# try:
#     from .defs import fmts
# except:
...
def create_packages(articles: list, orcid: str, client: MemberNodeClient_2_0, node: str):
    """
    ...
    try:
        for article in articles:
            ...
            lenfiles = len(files)
            L.info(f'Files: {lenfiles}')
            authors = article.get('authors')
            lenauthors = len(authors)
            for author in authors:
                given, family = parse_name(author.get('full_name'))
                if (given) and (family):
                    authors[author]['given_name'] = given
                    authors[author]['family_name'] = family
                else:
                    authors[author]['given_name'] = None
                    authors[author]['family_name'] = author.get('full_name')
            L.info(f'Authors: {lenauthors}')
            article['authors'] = authors
            try:
                xwalk = load_xwalk()
                article_eml = create_eml(article, xwalk)
                article_pid = create_package(orcid, doi, article, client)
                L.info(f'{doi} done. PID: {article_pid}')
...
def main():
    ...
    create_packages(qdcs=qdcs, orcid=orcid, client=client, node=node)
